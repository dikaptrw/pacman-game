...
import { createMaze, Cell, CellType, isWalkable, getCell } from '@/utils/mazeUtils';
import { Ghost, GhostType, GhostMode, createGhosts, getGhostColor, calculateGhostTarget } from '@/utils/ghostUtils';
...
interface GridGhost extends Omit<Ghost, 'x' | 'y'> {
  position: GridPosition;
...
const GameBoard: React.FC = () => {
  ...
  const { playSound, stopSound, toggleSounds, soundsEnabled } = useSounds();
  ...
  const gridToPixel = (position: GridPosition) => {
    return {
      x: position.col * CELL_SIZE,
      y: position.row * CELL_SIZE
    };
  };

  // Add a visual effect
  const addVisualEffect = (type: 'score' | 'ghost' | 'powerPellet' | 'death', position: GridPosition, value?: number) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!pacman.isMoving || 
          (e.key === 'ArrowUp' && pacman.direction === 'down') ||
          (e.key === 'ArrowDown' && pacman.direction === 'up') ||
          (e.key === 'ArrowLeft' && pacman.direction === 'right') ||
          (e.key === 'ArrowRight' && pacman.direction === 'left')) {
  ...
  }, [gameState, pacman.isMoving, pacman.direction, playSound, toggleSounds]);
  ...
  const movePacman = (deltaTime: number) => {
    setPacman(prev => {
      ...
      if (!prev.isMoving) {
        ...
        if (prev.nextDirection !== 'none' && canMove(prev.position, prev.nextDirection)) {
          return {
            ...prev,
        ...
        else if (prev.direction !== 'none' && canMove(prev.position, prev.direction)) {
          return {
            ...prev,
            isMoving: true,
            moveProgress: 0
      ...
      if (newProgress >= 1) {
        ...
        if (currentCell && (currentCell.type === 'dot' || currentCell.type === 'powerPellet')) {
          // Play sound
          playSound('munch');
          ...
          setMaze(prev => {
            const newMaze = [...prev];
            newMaze[newRow][newCol] = { ...newMaze[newRow][newCol], type: 'empty' };
            return newMaze;
          });
            ...
            const timer = setTimeout(() => {
              ...
              setGhosts(prevGhosts => 
                prevGhosts.map(ghost => {
                  ...
                  switch (ghost.type) {
                    case 'blinky': baseSpeed = GHOST_MOVE_TIME; break;
                    case 'pinky': baseSpeed = GHOST_MOVE_TIME * 1.05; break;
                    case 'inky': baseSpeed = GHOST_MOVE_TIME * 1.1; break;
                    case 'clyde': baseSpeed = GHOST_MOVE_TIME * 1.15; break;
                  }
      ...
      return prevGhosts.map(ghost => {
        ...
        if (!ghost.isMoving) {
          ...
          if (ghost.direction !== 'down' && 
              canMove(ghost.position, 'up')) {
            possibleDirections.push('up');
          }
          
          if (ghost.direction !== 'up' && 
              canMove(ghost.position, 'down')) {
            possibleDirections.push('down');
          }
          
          if (ghost.direction !== 'right' && 
              canMove(ghost.position, 'left')) {
            possibleDirections.push('left');
          }
          
          if (ghost.direction !== 'left' && 
              canMove(ghost.position, 'right')) {
          ...
          if (possibleDirections.length === 0) {
            ...
            if (canMove(ghost.position, opposite)) {
              possibleDirections.push(opposite);
            }
          }
  ...
  const checkGhostCollisions = () => {
    ghosts.forEach(ghost => {
      ...
      if (samePosition || 
          (pacman.isMoving && ghost.isMoving && 
  ...
  const areMovingBetweenSameCells = (pacman: Pacman, ghost: GridGhost): boolean => {
    ...
    switch (pacman.direction) {
      case 'up': pacmanTargetRow -= 1; break;
      case 'down': pacmanTargetRow += 1; break;
      case 'left': pacmanTargetCol -= 1; break;
      case 'right': pacmanTargetCol += 1; break;
    }
    ...
    switch (ghost.direction) {
      case 'up': ghostTargetRow -= 1; break;
      case 'down': ghostTargetRow += 1; break;
      case 'left': ghostTargetCol -= 1; break;
      case 'right': ghostTargetCol += 1; break;
    }
  ...
  useEffect(() => {
      ...
      row.forEach((cell, x) => {
        ...
        switch (cell.type) {
          ...
          case 'dot':
            ...
            ctx.beginPath();
            ctx.arc(xPos + CELL_SIZE / 2, yPos + CELL_SIZE / 2, 2, 0, Math.PI * 2);
            ctx.fill();
            break;
          case 'powerPellet':
            ...
            ctx.beginPath();
            ctx.arc(xPos + CELL_SIZE / 2, yPos + CELL_SIZE / 2, pulseSize, 0, Math.PI * 2);
            ctx.fill();
            break;
          default:
    ...
    ghosts.forEach(ghost => {
      ...
      if (ghost.mode !== 'frightened') {
        ...
        // Left pupil
        ctx.beginPath();
        ctx.arc(ghostX + radius - 2 + pupilOffsetX, ghostY + radius - 2 + pupilOffsetY, 1, 0, Math.PI * 2);
        ctx.fill();
    ...
    visualEffects.forEach(effect => {
      ...
      switch (effect.type) {
        case 'score':
          ...
          ctx.fillText(`${effect.value}`, effectX + CELL_SIZE / 2, effectY - 5 * progress);
          break;
        case 'ghost':
          ctx.fillStyle = `rgba(255, 255, 255, ${alpha})`;
          ctx.font = '14px Arial';
          ctx.textAlign = 'center';
          ctx.fillText(`${effect.value}`, effectX + CELL_SIZE / 2, effectY - 10 * progress);
          break;
        case 'powerPellet':
          ctx.fillStyle = `rgba(0, 255, 255, ${alpha})`;
          ctx.beginPath();
          const radius = 20 * progress;
          ctx.arc(effectX + CELL_SIZE / 2, effectY + CELL_SIZE / 2, radius, 0, Math.PI * 2);
          ctx.fill();
          break;
        case 'death':
          ...
          ctx.arc(effectX + CELL_SIZE / 2, effectY + CELL_SIZE / 2, deathRadius, 0, Math.PI * 2);
          ctx.fill();
          break;
      }
    });
    ...
    if (gameState === 'ready') {
      ...
      ctx.fillText('Press SPACE to start', canvas.width / 2, canvas.height / 2 + 24);
      ...
      ctx.fillText('Get ready for level ' + (level + 1), canvas.width / 2, canvas.height / 2 + 24);
    }
